BUILD = build
MAKEFILE = Makefile
OUTPUT_FILENAME = Neunzehn_Monate_Kommandant_SM_Kreuzer_Schwalbe
TITLE_NAME = "Neunzehn Monate Kommandant S.M. Kreuzer Schwalbe"
...
PREFACE_HTML_PDF = chapters/preface/preface_epub.md chapters/preface/00_Vorwort_epub.md
...
$(BUILD)/pdf/$(OUTPUT_FILENAME).pdf: $(MAKEFILE) $(METADATA_PDF) $(CHAPTERS_HTML_PDF) $(CSS_FILE) $(IMAGES) $(COVER_IMAGE) $(METADATA_PDF) $(PREFACE_HTML_PDF)
	mkdir -p $(BUILD)/pdf
	cp  *.css  $(IMAGES_FOLDER)
	pandoc $(ARGS_HTML) $(CSS_ARG_PRINT)  --extract-media=. --pdf-engine=prince --resource-path=$(IMAGES_FOLDER) --from markdown+pandoc_title_block+raw_html+fenced_divs+fenced_code_attributes+bracketed_spans+yaml_metadata_block --to=html5 -o $@ $(PREFACE_HTML_PDF) $(CHAPTERS)
	rm  $(IMAGES_FOLDER)/*.css
	rm *.jpg 
